...
published: true
slug: ugrep
title: ugrep bash script and documentation prompt
comment: https://github.com/Genivia/ugrep
---
# prompt
{% codeblock markdown %}
  <purpose>
    Generate Linux command-line invocations that use `ugrep` as the only external executable to achieve the user's objective.
    All command options and behaviors MUST be grounded strictly in the provided ugrep documentation in [[ugrep_documentation]].
    Output MUST be a single JSON object matching the required schema in <output_format_specification>.
  </purpose>

  <context>
    <audience>Anyone who needs reliable, doc-grounded ugrep commands.</audience>

    <operating_assumptions>
      <os>Linux</os>
      <shell_default>bash</shell_default>
      <default_search_root>.</default_search_root>
    </operating_assumptions>

    <constraints>
      <constraint>No external executables besides `ugrep` may be used. Do not use other commands (e.g., find, xargs, sed, awk, rg, grep, cat, python).</constraint>
      <constraint>Shell quoting is allowed. Do not pipe output into other commands.</constraint>
      <constraint>Use ONLY options, flags, and invocation forms explicitly present in [[ugrep_documentation]]. Do NOT guess or invent flags.</constraint>
      <constraint>If the objective requires an option/feature not present in [[ugrep_documentation]], you must return commands=[] and a specific non-empty error_message.</constraint>
      <constraint>Return exactly one command when possible. Return multiple commands only if one command cannot satisfy the objective while staying within constraints.</constraint>
      <constraint>Never output anything except the final JSON object specified in <output_format_specification>.</constraint>
    </constraints>
  </context>

  <instructions>
    <instruction>Parse [[user_objective]] and extract: (a) the search pattern(s), (b) target path(s) or whether a default is needed, (c) whether the pattern should be treated as regex or literal, (d) whether recursion is required, (e) desired output detail (e.g., filenames only, line numbers, counts, context, machine-readable), and (f) any file-type targeting requirements.</instruction>

    <instruction>Read [[ugrep_documentation]] and build an internal list of allowed: (a) command forms (how ugrep is invoked), (b) options/flags, and (c) any documented defaults/behaviors relevant to the objective.</instruction>

    <instruction>Decide whether the objective is achievable using ONLY the allowed items from the documentation. If not achievable, skip command generation and go directly to the failure output rule in the next instruction.</instruction>

    <instruction>If not achievable OR if critical details are missing (e.g., no pattern and none can be inferred; no target path and the documentation does not permit a safe default), output:
      {"commands":[],"explanation":"","error_message":"<specific reason grounded in the missing/unsupported requirements and/or absent documentation>"}.
      Do not add other keys.</instruction>

    <instruction>If achievable, generate the minimal `ugrep` command (or commands) that satisfies the objective, using only documented options and invocation forms. Prefer a single command that combines requirements when the documentation supports it.</instruction>

    <instruction>Write a brief explanation (1–3 sentences) describing how the command(s) fulfill the objective, without claiming undocumented behavior.</instruction>

    <instruction>Return ONLY the final JSON object exactly matching the schema in <output_format_specification>.</instruction>
  </instructions>
    ...
    <schema><![CDATA[
{
  ...
  "error_message": "<error message if the user's request cannot be fulfilled, else leave empty>"
    ...
    <rules>
      <rule>If fulfilled: "error_message" must be "" (empty string).</rule>
      <rule>If not fulfilled: "commands" must be [], and "error_message" must be non-empty and specific.</rule>
      <rule>Only include multiple commands if necessary; otherwise provide exactly one command.</rule>
      <rule>Commands must be directly executable `ugrep` invocations; do not include other commands.</rule>
    </rules>
  </output_format_specification>
  ...
  <examples>
    <example>
      <input_data>
        <ugrep_documentation><![CDATA[
Usage:
  ugrep [OPTIONS] PATTERN [FILE|DIR...]
Options:
  -n    print line numbers
  -i    ignore case
  -r    recurse into directories
        ]]></ugrep_documentation>
        <user_objective><![CDATA[
Search recursively under ./src for the case-insensitive pattern "TODO" and include line numbers.
        ]]></user_objective>
      </input_data>
      <output><![CDATA[
{
  "commands": [
    "ugrep -r -n -i \"TODO\" ./src"
  ],
  "explanation": "Runs ugrep recursively in ./src, matching \"TODO\" case-insensitively and printing line numbers for each match, using only options documented in the provided excerpt.",
  "error_message": ""
}
      ]]></output>
    </example>
...
Options:
  -n    print line numbers
        ...
        <user_objective><![CDATA[
Search recursively under ./docs for lines starting with "## " and output only matching filenames.
        ]]></user_objective>
      </input_data>
      <output><![CDATA[
{
  "commands": [],
  "explanation": "",
  "error_message": "Cannot fulfill the objective with the provided documentation excerpt: it does not document any recursive search option nor an option to output filenames only. Provide fuller ugrep documentation that includes the needed options, or revise the objective to match documented capabilities."
}
      ]]></output>
    </example>
  </examples>
