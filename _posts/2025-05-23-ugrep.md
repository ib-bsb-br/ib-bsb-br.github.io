...
MASTER_OUTPUT_ROOT="./batch_search_run_$(date +%Y%m%d_%H%M%S)"
...
MAX_CONCURRENT_JOBS=${MAX_CONCURRENT_JOBS:-$(nproc --all || echo 7)} # Default to number of processors or 4
active_jobs=0

# --- ugrep Configuration ---
UGREP_CMD="ug+" # or "ug" if ugrep filters are globally configured (e.g. in ~/.ugrep)
UGREP_OPTS_BASE="-r -i -l" # Recursive, case-insensitive, list filenames
UGREP_OPTS_ARCHIVES="-z"   # Search archives
# UGREP_OPTS_INDEX="--index" # Add this if you have pre-built ugrep indexes for search directories
...
declare -a SEARCH_LOCATIONS=(
    ...
    "/mnt/external_hdd/7z/"
    "/mnt/external_hdd/apk/"
    "/mnt/external_hdd/aux/"
    "/mnt/external_hdd/bak/"
    "/mnt/external_hdd/bcf/"
    "/mnt/external_hdd/bib/"
    "/mnt/external_hdd/cab/"
    "/mnt/external_hdd/crate/"
    "/mnt/external_hdd/cs/"
    "/mnt/external_hdd/csv/"
    "/mnt/external_hdd/dat/"
    "/mnt/external_hdd/db/"
    "/mnt/external_hdd/db3/"
    "/mnt/external_hdd/dic/"
    "/mnt/external_hdd/dll/"
    "/mnt/external_hdd/dmp/"
    "/mnt/external_hdd/docx/"
    "/mnt/external_hdd/gz/"
    "/mnt/external_hdd/h/"
    "/mnt/external_hdd/iso/"
    "/mnt/external_hdd/jar/"
    "/mnt/external_hdd/json/"
    "/mnt/external_hdd/jsonl/"
    "/mnt/external_hdd/log/"
    "/mnt/external_hdd/md/"
    "/mnt/external_hdd/mobi/"
    "/mnt/external_hdd/model/"
    "/mnt/external_hdd/mov/"
    "/mnt/external_hdd/mp4/"
    "/mnt/external_hdd/no_extension/"
    "/mnt/external_hdd/ods/"
)
...
# Function to sanitize path components for use in filenames/directory names
sanitize_for_path() {
    echo "$1" | sed 's#^/*##;s#/*$##;s#/#_#g' | tr ' ' '_' | sed 's/[^a-zA-Z0-9_-]/_/g'
}

# --- Search Task Function ---
# This function performs a single search task for a given location and query.
# It's designed to be run in the background.
perform_search_task() {
    local search_dir_param="$1"
    local query_param="$2"
    local task_id_suffix="$3" # Unique suffix for this task (e.g., from PID or counter)

    # Sanitize parameters for path creation
    local sane_loc_part=$(sanitize_for_path "$(basename "$search_dir_param")")
    if [ -z "$sane_loc_part" ]; then # Handle cases like "/" if ever passed
        sane_loc_part="root"
    fi
    local sane_query_part=$(sanitize_for_path "$query_param")

    local task_identifier="loc_${sane_loc_part}_query_${sane_query_part}"

    # Define task-specific paths
    local task_results_dir="${ALL_RESULTS_PARENT_DIR}/${task_identifier}"
    local task_log_file="${ALL_LOGS_PARENT_DIR}/log_${task_identifier}.txt"
    # local task_ocr_temp_dir="${OCR_BASE_TEMP_DIR}/${task_identifier}_${task_id_suffix}" # If using OCR

    # Ensure task-specific directories exist
    mkdir -p "$task_results_dir"
    # mkdir -p "$task_ocr_temp_dir" # Uncomment if using OCR

    # Task-specific log function
    task_log() {
        echo "$(date '+%Y-%m-%d %H:%M:%S') [TASK: ${task_identifier}] - $1" >> "$task_log_file"
    }

    # Start task logging
    echo "--- Log for Query: '$query_param' in Location: '$search_dir_param' (Task ID Suffix: $task_id_suffix) ---" > "$task_log_file"
    task_log "Starting search task."
    task_log "Results will be in: $task_results_dir"
    task_log "Full log at: $task_log_file"

    # Define search paths for this task. Default is just the search directory.
    local current_search_paths="$search_dir_param"

    # --- Optional: OCR Pre-processing Step (Commented Out) ---
    # If uncommented, ensure 'pdftotext' and 'tesseract' are installed.
    # The OCR logic here is a basic placeholder and may need significant enhancement
    # for robust error handling, efficiency, and accuracy in a real-world scenario.
    # Consider that OCR is CPU-intensive and will impact overall performance.
    #
    # task_log "OCR pre-processing (if applicable)..."
    # find "$search_dir_param" -type f -iname "*.pdf" -print0 | while IFS= read -r -d $'\0' pdf_file; do
    #   # Basic heuristic: if pdftotext extracts very little, assume it might need OCR
    #   # This heuristic is very basic and might misclassify PDFs.
    #   text_content_check=$(pdftotext "$pdf_file" -enc UTF-8 - | tr -d '[:space:]')
    #   if [ ${#text_content_check} -lt 100 ]; then # Arbitrary threshold
    #     task_log "Potential scanned PDF (or PDF with minimal text): $pdf_file. Attempting OCR."
    #     ocr_output_basename=$(basename "$pdf_file" .pdf)
    #     # Ensure unique output name for OCRed text file, placed in task-specific OCR temp dir
    #     # tesseract "$pdf_file" "${task_ocr_temp_dir}/${ocr_output_basename}" -l eng >> "$task_log_file" 2>&1
    #     # if [ $? -eq 0 ]; then
    #     #   task_log "OCR successful for $pdf_file"
    #     # else
    #     #   task_log "OCR failed for $pdf_file"
    #     # fi
    #   fi
    # done
    # task_log "OCR pre-processing finished."
    # # If OCR was performed, add the directory with OCRed text files to the search paths:
    # # current_search_paths="$search_dir_param $task_ocr_temp_dir"
    # --- End of Optional OCR Section ---

    local output_file_matches="$task_results_dir/matches.txt"
    
    task_log "Executing ugrep: $UGREP_CMD $UGREP_OPTS_BASE $UGREP_OPTS_ARCHIVES \"$query_param\" $current_search_paths"

    # Execute ugrep. Redirect stderr to task log.
    # Add $UGREP_OPTS_INDEX to the command below if you are using indexed search.
    if $UGREP_CMD $UGREP_OPTS_BASE $UGREP_OPTS_ARCHIVES "$query_param" $current_search_paths > "$output_file_matches" 2>> "$task_log_file"; then
        if [ -s "$output_file_matches" ]; then
            task_log "SUCCESS: Matches found. Results are in $output_file_matches"
        else
            task_log "INFO: No matches found."
            rm "$output_file_matches" # Optional: remove empty result files
        fi
    else
        local ugrep_exit_code=$?
        task_log "ERROR: ugrep command failed with exit code $ugrep_exit_code."
        # Optionally remove the output file if ugrep failed, as it might be empty or incomplete
        # rm -f "$output_file_matches"
    fi
    
    task_log "Search task finished."
    # rm -rf "$task_ocr_temp_dir" # Uncomment if using OCR and want to clean up temp OCR files
}
export -f perform_search_task sanitize_for_path # Export functions for subshells if needed by `bash -c` or similar

# --- Main Orchestration ---

# Create base directories
mkdir -p "$ALL_RESULTS_PARENT_DIR"
if [ $? -ne 0 ]; then echo "FATAL: Could not create results parent directory '$ALL_RESULTS_PARENT_DIR'. Exiting." >&2; exit 1; fi
mkdir -p "$ALL_LOGS_PARENT_DIR"
if [ $? -ne 0 ]; then echo "FATAL: Could not create logs parent directory '$ALL_LOGS_PARENT_DIR'. Exiting." >&2; exit 1; fi
# mkdir -p "$OCR_BASE_TEMP_DIR" # Uncomment if using OCR
...
master_log "Assumptions: ugrep ('$UGREP_CMD') is installed. Read access to search locations. Write access to output directory."
master_log "For OCR (currently commented out): pdftotext and tesseract would be required."

total_tasks=0
declare -a pids_list=() # To store PIDs of backgrounded tasks
...
for location in "${SEARCH_LOCATIONS[@]}"; do
    for query in "${SEARCH_QUERIES[@]}"; do
        ...
        # Manage concurrency: if max jobs are running, wait for one to finish
        if [ "$active_jobs" -ge "$MAX_CONCURRENT_JOBS" ]; then
            master_log "Reached max concurrent jobs ($MAX_CONCURRENT_JOBS). Waiting for a slot..."
            wait -n # Waits for any background job to complete
            if [ $? -eq 0 ]; then # Check if wait was successful (a job finished)
                 active_jobs=$((active_jobs - 1))
            else # wait -n can return >128 if no children or interrupted by signal
                 # This part might need more robust handling for different OS `wait -n` behaviors
                 # For now, assume a job slot freed up or try to find finished jobs
                 # A more robust way is to check specific PIDs, but `wait -n` is simpler for general load.
                 # Let's just decrement optimistically or re-evaluate active PIDs.
                 # For simplicity, we'll just decrement. A more complex solution would track PIDs carefully.
                 active_jobs=$((active_jobs -1)) 
                 if [ "$active_jobs" -lt 0 ]; then active_jobs=0; fi # Prevent negative
            fi
        fi
        
        master_log "Launching task $total_tasks: Query ['$query'] in Location ['$location']"
        
        # Launch the task in the background
        # The task_id_suffix could be $total_tasks or $$ for the subshell PID
        perform_search_task "$location" "$query" "$total_tasks" &
        pids_list+=($!) # Store PID of the last backgrounded process
        active_jobs=$((active_jobs + 1))
        
    done # End query loop
done # End location loop

master_log "All $total_tasks tasks have been launched."
master_log "Waiting for remaining $active_jobs active jobs to complete..."

# Wait for all remaining background jobs to finish
while [ "$active_jobs" -gt 0 ]; do
    wait -n
    exit_status=$?
    if [ $exit_status -eq 0 ] || [ $exit_status -eq 127 ]; then # 127 can mean no more children
        active_jobs=$((active_jobs - 1))
    else
        # Handle other exit statuses if necessary, or just decrement
        active_jobs=$((active_jobs - 1)) # Decrement anyway to avoid infinite loop on weird `wait -n` behavior
    fi
    if [ "$active_jobs" -lt 0 ]; then active_jobs=0; fi # Safety
    master_log "A job finished. Remaining active jobs: $active_jobs"
done

# Final wait, just in case (some systems/shells might behave differently with wait -n)
wait

master_log "All search tasks have completed."
master_log "Please check individual logs in: $ALL_LOGS_PARENT_DIR"
master_log "And results in: $ALL_RESULTS_PARENT_DIR"
