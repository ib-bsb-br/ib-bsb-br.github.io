...
#          is given, they are bundled into a temporary TAR archive, which is then synced.
#          Extraction is attempted for local destinations.
...
cesto() {
  ...
  if [ "$#" -lt 2 ]; then
    ...
    echo "Example (multiple files): cesto file1.txt docs/*.pdf ./backup/archived_files" >&2
    echo "Example (single dir): cesto ./my_project_source/ ./backup_location/project_data" >&2
    return 1
  fi

  local user_dest_dir="${!#}" # Last argument is the destination
  local sources_array=("${@:1:$#-1}") # All args except the last are sources
  local num_sources="${#sources_array[@]}"

  # fpsync configurations from your original request
  local num_workers=4
  local fpsync_opts="-v"

  # Path to the temporary tar file created on the source; initialized for cleanup trap
  # This variable is local to this function call.
  local tmp_source_tar_file_path=""

  # Setup a trap to clean up the temporary source tar file on exit or signal.
  # The command string is in single quotes, so $tmp_source_tar_file_path is evaluated when the trap runs.
  # The 'trap - ...' part attempts to make the trap self-disabling after its first execution.
  trap 'if [ -n "$tmp_source_tar_file_path" ] && [ -f "$tmp_source_tar_file_path" ]; then echo "INFO: (Trap) Cleaning up temporary source tar: $tmp_source_tar_file_path"; rm -f "$tmp_source_tar_file_path"; fi; trap - EXIT HUP INT QUIT TERM' EXIT HUP INT QUIT TERM
  ...
  local abs_dest_dir # Will hold the absolute path to the destination directory
  ...
  # This is the directory INTO which fpsync will place files or the tarball.
  local final_dest_dir_for_fpsync="${abs_dest_dir%/}/"


  # --- Source Path Handling ---
  local path_to_sync_via_fpsync # This will be the path (dir or tar file) fpsync uses as its source

  # Case 1: Single source item, AND it's a directory (after resolving symlinks)
  if [ "$num_sources" -eq 1 ]; then
    local single_src_item_orig="${sources_array[0]}"
    local single_src_item_abs # Will hold absolute path of the single source
    ...
    if [ -d "$single_src_item_abs" ]; then
      echo "INFO: Mode: Single source directory detected. Preparing for direct fpsync."
      path_to_sync_via_fpsync="${single_src_item_abs%/}/" # Ensure trailing slash
    # else: single source item is a file, path_to_sync_via_fpsync remains unset
    # and will be handled by the tar aggregation logic below.
    fi
  fi

  # Case 2: Multiple sources OR a single source item that is a file
  # (i.e., path_to_sync_via_fpsync is still unset).
  # Sources will be aggregated into a temporary TAR archive.
  if [ -z "$path_to_sync_via_fpsync" ]; then
    echo "INFO: Mode: Multiple sources or single file source detected. Aggregating into a temporary TAR archive for fpsync."
    
    # Attempt to create a temporary file for the tar archive
    tmp_source_tar_file_path=$(mktemp "${TMPDIR:-/tmp}/cesto_aggregate.XXXXXXXXXX.tar")
    # Check mktemp's success and if the file was created.
    if [ $? -ne 0 ] || [ -z "$tmp_source_tar_file_path" ] || [ ! -f "$tmp_source_tar_file_path" ]; then
      echo "Error: Failed to create temporary source TAR archive path using mktemp." >&2
      tmp_source_tar_file_path="" # Ensure var is empty so trap does nothing for this path
      return 1
    fi
    # The trap is already set and will use the new value of tmp_source_tar_file_path when it executes.

    local files_to_tar_array=()
    local item_prepared_for_tar=0
    for src_item_orig in "${sources_array[@]}"; do
      local src_item_abs # Will hold absolute path of the current source item
      ...
      if [ ! -e "$src_item_abs" ]; then # Check if it exists after full resolution
        echo "Warning: Source item \"$src_item_orig\" (resolved to \"$src_item_abs\") does not exist. Skipping." >&2
        continue
      fi
      # If a source item is a directory, tar will include it recursively.
      files_to_tar_array+=("$src_item_abs")
      item_prepared_for_tar=1
    done

    if [ "$item_prepared_for_tar" -eq 0 ]; then
      echo "Error: No valid source items were found to add to the TAR archive." >&2
      # The trap will clean up the empty mktemp-created tar file.
      return 1
    fi

    echo "INFO: Creating temporary TAR archive: ${tmp_source_tar_file_path}"
    echo "      Archiving the following absolute paths (leading '/' typically stripped by GNU tar within archive):"
    printf "      - %s\n" "${files_to_tar_array[@]}"
    
    # Using absolute paths; GNU tar by default strips leading '/' when archiving.
    # e.g., /home/user/file.txt becomes home/user/file.txt in the archive.
    # This is generally desirable for extraction into a target directory.
    if ! tar -cf "$tmp_source_tar_file_path" "${files_to_tar_array[@]}"; then
      echo "Error: Failed to create temporary TAR archive with source items." >&2
      # The trap will attempt to clean up tmp_source_tar_file_path.
      return 1
    fi
    path_to_sync_via_fpsync="$tmp_source_tar_file_path"
  fi

  # --- Execution ---
  echo "INFO: Running fpsync: \"${path_to_sync_via_fpsync}\" -> \"${final_dest_dir_for_fpsync}\""
  # If path_to_sync_via_fpsync is a tar file, fpsync transfers it into final_dest_dir_for_fpsync
  # If path_to_sync_via_fpsync is a dir/, fpsync syncs its content into final_dest_dir_for_fpsync
  ...
  if [ ${fpsync_status} -ne 0 ]; then
    ...
    if [ -n "$tmp_source_tar_file_path" ] && [ -f "$path_to_sync_via_fpsync" ]; then # Check if we were in tar mode
        local tar_basename # For messaging about destination
        tar_basename=$(basename "$tmp_source_tar_file_path")
        echo "INFO: The source tar archive \"${tar_basename}\" may have been partially transferred or may exist at the destination."
    fi
  else
    echo "INFO: fpsync operation completed successfully."
    # If a temporary source TAR was used (i.e., path_to_sync_via_fpsync == tmp_source_tar_file_path),
    # attempt to extract it on the destination.
    if [ -n "$tmp_source_tar_file_path" ] && [ "$path_to_sync_via_fpsync" == "$tmp_source_tar_file_path" ]; then
      local tar_basename # Basename of the source tar file
      tar_basename=$(basename "$tmp_source_tar_file_path")
      # Expected full path of the tar file on the destination side
      local dest_tar_archive_full_path="${final_dest_dir_for_fpsync%/}/${tar_basename}"

      # Basic check if destination path looks remote (user@host:path or protocol://...)
      if [[ "$user_dest_dir" == *@*:* ]] || [[ "$user_dest_dir" == *://* ]]; then
        echo "INFO: Destination \"$user_dest_dir\" appears to be remote."
        echo "      IMPORTANT: Please manually extract \"${tar_basename}\" within directory \"${final_dest_dir_for_fpsync}\""
        echo "      on the destination host, and then remove the tarball \"${dest_tar_archive_full_path}\" from the destination if desired."
      else
        ...
        if [ -f "$dest_tar_archive_full_path" ]; then
          ...
          # Using -C "${final_dest_dir_for_fpsync%/}" ensures paths from tar are relative to this dir.
          echo "      Extracting using: tar -xf \"${dest_tar_archive_full_path}\" -C \"${final_dest_dir_for_fpsync%/}\""
          if tar -xf "$dest_tar_archive_full_path" -C "${final_dest_dir_for_fpsync%/}"; then
            ...
            echo "INFO: Removing temporary archive from destination: \"${dest_tar_archive_full_path}\""
            if ! rm -f "$dest_tar_archive_full_path"; then
              echo "Warning: Failed to remove archive from destination: \"${dest_tar_archive_full_path}\"."
            fi
          else
            echo "Error: Failed to extract archive at destination. Archive remains: \"${dest_tar_archive_full_path}\"."
          fi
        else
          echo "Error: Transferred TAR archive not found at destination for extraction: \"${dest_tar_archive_full_path}\"."
          echo "       This might indicate that fpsync failed to transfer the file completely or an issue with paths."
        fi
      fi
    fi
  fi
  
  # The EXIT trap will handle cleaning $tmp_source_tar_file_path (source side) if it was set and the file exists.
