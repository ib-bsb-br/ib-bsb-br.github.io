...
PROC_ARGS_FULL=$(echo "$PROC_CMD_LINE_RAW" | sed -e 's/^[^ ]* *[^ ]* *[^ ]* *[^ ]* *[^ ]* *//' -e 's/^[ \t]*//' || echo "Arguments not retrieved")
if echo "$PROC_ARGS_FULL" | grep -q "pure-ftpd process not found"; then PROC_ARGS_FULL="pure-ftpd process not found or args not retrieved"; fi


LISTENING_PORTS=$(ss -tlpn 2>/dev/null | grep 'pure-ftpd' | awk '{print $4}' | sed 's/.*://' | sort -u | tr '\n' ' ' | sed 's/ *$//g')
if [ -z "$LISTENING_PORTS" ]; then LISTENING_PORTS="Not found via ss or pure-ftpd not listening"; fi

PASSIVE_PORTS_CONF_VAL="Not set or file not found"
...
if getent passwd ftpuser >/dev/null 2>&1; then
    ...
    FTPUSER_HOME=$(getent passwd ftpuser | cut -d: -f6)
    FTPUSER_SHELL=$(getent passwd ftpuser | cut -d: -f7)
fi

# Main echo command using the pre-calculated variables
echo -e "--- FTP Server Details (pure-ftpd on Debian Bullseye) ---\n\
\n**General System & Service Info:**\n\
  System IP Addresses: $IP_ADDRESSES\n\
  Pure-FTPd Service Status (systemctl): $SERVICE_STATUS\n\
  Pure-FTPd Package Version (dpkg): $PKG_VERSION\n\
  Running pure-ftpd Command Line (best effort from ps): $PROC_ARGS_FULL\n\
  Listening FTP Port(s) (from ss): $LISTENING_PORTS (Typically 21 for control)\n\
\n**Configuration (Values from /etc/pure-ftpd/conf/ unless overridden by command line args above):**\n\
  Passive Port Range (from /etc/pure-ftpd/conf/PassivePortRange): $PASSIVE_PORTS_CONF_VAL\n\
  Anonymous Login (from /etc/pure-ftpd/conf/NoAnonymous): $NO_ANON_CONF_MSG\n\
  TLS Mode (from /etc/pure-ftpd/conf/TLS): $TLS_CONF_MSG\n\
  TLS Certificate: Path is typically set via a command-line option (e.g., -T /path/to/cert.pem) or uses a default compiled-in path. Check the running process arguments above. The tutorial uses /etc/ssl/private/pure-ftpd.pem.\n\
  Chroot Behavior: Default is often to chroot users. Check for /etc/pure-ftpd/conf/NoChroot (if present, may disable chroot for some) or -A/-a flags in running command line args above.\n\
\n**FTP User Information (based on tutorial's 'ftpuser'):**\n\
  FTP User for Connection: System user (the tutorial guides creating 'ftpuser').\n\
  Password for FTP User: [This cannot be retrieved. Use the password you set for the system user, e.g., 'ftpuser'.]\n\
  Status of 'ftpuser': $FTPUSER_EXISTS_MSG\n\
  Home Directory for 'ftpuser' (if exists): $FTPUSER_HOME\n\
  Shell for 'ftpuser' (if exists): $FTPUSER_SHELL\n\
\n**Firewall (UFW Example - run the check command with sudo for current status):**\n\
  Expected Ports (as per tutorial): 21/tcp (FTP Control), 30000-35000/tcp (Passive FTP)\n\
  To check UFW status for these ports, run: sudo ufw status verbose | grep -E '21/tcp|30000:35000/tcp'\n\
-------------------------------------------------"
