...
| `--template-file`   | file path / None     | `-t`  | Uses a lightweight token template for custom formatting (`{{ path }}`, `{{ content }}`, `{{ index }}`).   |
...
3. **Template File (`-t`/`--template-file`):**
...
function Invoke-PathProcessing {
    ...
    Get-ChildItem -LiteralPath $CurrentPath -Force | Sort-Object FullName | ForEach-Object {
        ...
        if ($Ignore.Count -gt 0 -and ($Ignore | Where-Object { $itemPath -like $_ -or $_.Name -like $_ })) {
            return
        }
