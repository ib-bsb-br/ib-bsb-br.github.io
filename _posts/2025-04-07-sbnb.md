...
run_advanced_mode() {
    ...
    prepare_usb_advanced() {
        ...
        local root_dev_path_adv_raw; root_dev_path_adv_raw=$(findmnt -n -o SOURCE /)
        local resolved_root_path; resolved_root_path=$(realpath "$root_dev_path_adv_raw" 2>/dev/null)
        local root_fs_block_device_for_pkname=""

        if [ -n "$resolved_root_path" ] && [ -b "$resolved_root_path" ]; then
            root_fs_block_device_for_pkname="$resolved_root_path"
        else
            local df_root_src; df_root_src=$(df --output=source / | awk 'NR==2 {print $1}')
            local resolved_df_root_src; resolved_df_root_src=$(realpath "$df_root_src" 2>/dev/null)
            if [ -n "$resolved_df_root_src" ] && [ -b "$resolved_df_root_src" ]; then
                info "Root FS source '$root_dev_path_adv_raw' (resolved: '${resolved_root_path:-<empty/failed>}') not a direct block device. Using 'df /' source '$df_root_src' (resolved: '$resolved_df_root_src') for pkname lookup."
                root_fs_block_device_for_pkname="$resolved_df_root_src"
            else
                error "Cannot determine a valid block device for the root filesystem to find its PKNAME. Sources checked: findmnt raw '$root_dev_path_adv_raw' (resolved '${resolved_root_path:-<empty/failed>}'), df raw '$df_root_src' (resolved '${resolved_df_root_src:-<empty/failed>}')."
            fi
        fi
        local root_base_dev_name_adv; root_base_dev_name_adv=$(lsblk -no pkname "$root_fs_block_device_for_pkname")
        if [ -z "$root_base_dev_name_adv" ]; then # Check if lsblk succeeded in getting a pkname
            error "Failed to get PKNAME for root filesystem device '$root_fs_block_device_for_pkname'. Safety check aborted."
        fi

        local resolved_target_path; resolved_target_path=$(realpath "$TARGET_DEVICE_ADV" 2>/dev/null)
        if [ -z "$resolved_target_path" ] || [ ! -b "$resolved_target_path" ]; then
            error "Selected target device '$TARGET_DEVICE_ADV' (resolved: '${resolved_target_path:-<empty/failed>}') is not a valid block device."
        fi
        local target_base_dev_name_adv; target_base_dev_name_adv=$(lsblk -no pkname "$resolved_target_path")
        if [ -z "$target_base_dev_name_adv" ]; then # Check if lsblk succeeded
            error "Failed to get PKNAME for target device '$resolved_target_path'. Safety check aborted."
        fi
        
        local root_base_dev_adv="/dev/${root_base_dev_name_adv}"
        local target_base_dev_adv="/dev/${target_base_dev_name_adv}"

        if [ "$target_base_dev_adv" == "$root_base_dev_adv" ]; then
          error "FATAL ERROR: Target device '$TARGET_DEVICE_ADV' (base: $target_base_dev_adv) appears to be the same device as the running root filesystem (base: $root_base_dev_adv, determined from source: '$root_fs_block_device_for_pkname', original findmnt source: '$root_dev_path_adv_raw'). Aborting."
        fi
        info "Safety check passed: Target device '$TARGET_DEVICE_ADV' (base: $target_base_dev_adv) is not the root filesystem device (base: $root_base_dev_adv)."
...
if [ "$current_free_percentage" -lt "$MINIMUM_FREE_SPACE_PERCENTAGE" ]; then
    ...
    if [ {VOLUME_CHECK_PRUNE_LEVEL_CONFIG} -gt 0 ]; then
        ...
        prune_operation_full_output=$(eval "${{DOCKER_PRUNE_COMMAND_TO_RUN}}" 2>&1) || prune_command_exit_code=$?
...
EOF_PYTHON_SCRIPT
    ...
    if [[ "$tailscale_auth_key_for_python" == "tskey-auth-PLACEHOLDER-EDIT-THIS-FILE-MANUALLY" ]]; then
        echo -e "${RED} CRITICAL REMINDER: You used a placeholder Tailscale key. Edit '${esp_mount_point_adv}/sbnb-tskey.txt' MANUALLY with your actual key! ${NC}"
    fi
    echo -e "${BLUE}====================================================${NC}"
