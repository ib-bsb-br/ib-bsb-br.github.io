module Jekyll
  class CodeBlockTag < Liquid::Block
    ...
    def render(context)
      ...
      <<~HTML
        <pre data-filename=\"#{filename}\" data-code=\"#{escaped_code_content}\" data-download-link data-download-link-label=\"#{download_label}\"><code class=\"language-#{@language}\">#{code_content}</code></pre>
      HTML
    end
  end
end
